...
const METRICS: MetricConfig[] = [
  { key: "heartRate", labelKey: "heartRateLabel", unit: "bpm", color: "hsl(172, 66%, 30%)", getValues: e => e.heartRate },
  { key: "bloodPressure", labelKey: "bloodPressure", unit: "mmHg", color: "hsl(215, 16%, 47%)", getValues: e => e.systolic },
  { key: "weight", labelKey: "weightLabel", unit: "kg", color: "hsl(172, 40%, 45%)", getValues: e => e.weight },
  { key: "peakFlow", labelKey: "peakFlowLabel", unit: "L/min", color: "hsl(172, 66%, 25%)", getValues: e => e.peakFlow },
];
...
const HealthCharts = ({ entries, quitDate }: HealthChartsProps) => {
  ...
  const selectedLabel = (t as any)[selectedMetric.labelKey] as string;
    ...
    <div className="card-elevated p-5 space-y-4">
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
        {availableMetrics.map(m => {
          const label = (t as any)[m.labelKey] as string;
          return (
            <button
              key={m.key}
      ...
      {chartData.length >= 2 ? (
        <div className="h-40">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <YAxis domain={["auto", "auto"]} tick={{ fontSize: 10, fill: "hsl(215, 16%, 47%)" }} axisLine={false} tickLine={false} />
              <Tooltip
                contentStyle={{ background: "hsl(0, 0%, 100%)", border: "none", borderRadius: "12px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", fontSize: "12px", padding: "8px 12px" }}
                formatter={(value: number) => [`${value} ${selectedMetric.unit}`, selectedLabel]}
                labelFormatter={(label) => label}
              />
              <Area type="monotone" dataKey="value" stroke={selectedMetric.color} strokeWidth={2} fill="url(#chartGradient)" dot={{ r: 3, fill: selectedMetric.color, strokeWidth: 0 }} activeDot={{ r: 5, fill: selectedMetric.color, strokeWidth: 2, stroke: "white" }} />
              {activeMetric === "bloodPressure" && (
                <Area type="monotone" dataKey="diastolic" stroke="hsl(215, 16%, 67%)" strokeWidth={1.5} fill="none" dot={{ r: 2, fill: "hsl(215, 16%, 67%)", strokeWidth: 0 }} strokeDasharray="4 2" />
              )}
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="flex h-32 items-center justify-center">
          <p className="text-xs text-muted-foreground">{t.logAtLeast2}</p>
        </div>
      )}
      ...
      {chartData.length > 0 && (
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-[0.6rem] text-muted-foreground/60 ml-auto">
            {t.latest} · {chartData[chartData.length - 1].date}
          </span>
        </div>
      )}
    </div>
